    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                TimeSpan lag1 = System.DateTime.Now - rep.GetTime();
                TimeSpan lag2 = rep.GetTime() - ts_prev;

                DebugPrint(
                    string.Format(
                        "Dequeue. Queue length: {0}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3} ms. Lag between packets: {4} ms",
                        reports.Count,
                        report_buf[0],
                        report_buf[1],
                        lag1.TotalMilliseconds,
                        lag2.TotalMilliseconds
                    ),
                    DebugType.THREADING
                );
